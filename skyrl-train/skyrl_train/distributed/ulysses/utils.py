...
# NOTE (sumanthrh): This is an extension of the same helper function in VERL.
# We further add attention_mask slicing
def ulysses_pad_and_slice_inputs(
    ...
    # sp_size > 1. assert that a group exists
    ...
    if attention_mask_rmpad is not None:
        attention_mask_rmpad = slice_input_tensor(attention_mask_rmpad, dim=1, padding=False)
    return input_ids_rmpad, position_ids_rmpad, attention_mask_rmpad, pad_size
